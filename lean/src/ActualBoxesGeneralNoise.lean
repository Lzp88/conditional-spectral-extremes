...
/-! Full bounded-noise version of the manuscript's constrained-kernel
lower bound. The noise vector may have any probability distribution;
independence between its coordinates is not needed for this bound. -/
...
theorem actual_regular_reference_path_mass_lower_bounded_noise (κmin κmax K_E C_E : Real)
    ...
    ∃ B₀ D₀ Cstar : Real, 0 < B₀ ∧ 0 < D₀ ∧ 0 < Cstar ∧
      ...
      ∀ᶠ n : Nat in atTop, ∀ κ ∈ Icc κmin κmax, ∀ η : Real, ∀ q : Nat → Nat,
        ...
        ∀ noise : Measure (Fin (count p n) → ℝ), IsProbabilityMeasure noise →
          (∀ᵐ ζ ∂noise, ∀ i, |ζ i| ≤ (L n)^(-10 : Real)) →
          (L n)^(-Cstar) ≤ (referencePathMass p n κ (gStar*ell n) q noise).toReal := by
  ...
  intro κ hκ η q hreg noise hprob hsupport
  let _ := hprob
  ...
  have hinner := referencePathMass_lower_inner p n κ (gStar*ell n) ((L n)^(-10 : Real)) q hk
    hnoise.1.le hnoise.2.1 hnoise.2.2 noise hsupport
...
#print axioms actual_regular_reference_path_mass_lower_bounded_noise
