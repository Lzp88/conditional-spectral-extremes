import ActualKernelL1

/-! Genuine one- and two-point L1 density bounds from the actual Diophantine hypotheses. -/
...
theorem uniform_actual_l1_one (pmin P alpha : Real) (hpP : pmin ≤ P)
  ...
  apply actual_kernel_l1_one s P δ T K _ hs hsP hδ hT hK (by positivity) hsmall
    t m n q (by linarith)
  intro u hu
...
theorem uniform_actual_l1_two (pmin P alpha : Real) (hpP : pmin ≤ P)
    (ha : 0 < alpha) (hap : alpha < pmin) (ha1 : alpha ≤ 1) :
    ∃ C : Real, 0 < C ∧ ∀ (s δ T K R a Delta : Real) (m n q : Nat) (t₁ t₂ : AddCircle (1 : Real)),
      ...
      (∫ x : PairSpace, |pairSmoothedDensity δ
        (pairVectorLaw (harmonicTiltSumLaw s ![t₁,t₂] m n q)) x -
  ...
  have hs : 0 < s := by linarith
  apply actual_kernel_l1_two s P δ T K _ hs hsP hδ hT hK (by positivity) hsmall
...
#print axioms uniform_actual_l1_one
#print axioms uniform_actual_l1_two
