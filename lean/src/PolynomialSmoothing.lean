import DiophantineSmoothing
import NormalizerScaleError

/-! Polynomially accurate comparison for actual one- and two-point sampled height laws. -/
...
def PolynomialSmoothingOne (p P J _u₁ u₂ u₃ x : Real) : Prop :=
  ∀ (s a : Real) (m n q : Nat) (t : AddCircle (1 : Real)),
    ...
    totalVariationDistance
      ((coordinateLaw (harmonicTiltSumLaw s (fun _ : Fin 1 => t) m n q) 0) ∗ tenUniformNoise (x^(-10 : Real)))
      ((coordinateLaw (referenceVectorSumLaw s 1 q) 0) ∗ tenUniformNoise (x^(-10 : Real))) ≤ x^(-J) ∧
    |harmonicTiltNormalizer s (fun _ : Fin 1 => t) m n^q/logSineA s^q-1| ≤ x^(-J)

def PolynomialSmoothingTwo (p P J _u₁ u₂ u₃ x : Real) : Prop :=
  ∀ (s a : Real) (m n q : Nat) (t₁ t₂ : AddCircle (1 : Real)),
    ...
    pairTotalVariationDistance
      ((pairVectorLaw (harmonicTiltSumLaw s ![t₁,t₂] m n q)) ∗ pairUniformNoise (x^(-10 : Real)))
      ((pairVectorLaw (referenceVectorSumLaw s 2 q)) ∗ pairUniformNoise (x^(-10 : Real))) ≤ x^(-J) ∧
    |harmonicTiltNormalizer s ![t₁,t₂] m n^q/logSineA s^(2*q)-1| ≤ x^(-J)

theorem polynomial_smoothing_one (p P J α : Real) (hp : 0 < p) (hP : p ≤ P)
    (hJ : 0 < J) (hα : 0 < α) (hαp : α < p) (hα1 : α ≤ 1) :
    ∀ᶠ x : Real in atTop, PolynomialSmoothingOne p P J
      (smoothingExponentOne J) (smoothingExponentTwo J α) (smoothingExponentThree J) x := by
  obtain ⟨C,hC,hTV⟩ := uniform_actual_smoothing_one p P α hP hα hαp hα1
...
theorem polynomial_smoothing_two (p P J α : Real) (hp : 0 < p) (hP : p ≤ P)
    (hJ : 0 < J) (hα : 0 < α) (hαp : α < p) (hα1 : α ≤ 1) :
    ∀ᶠ x : Real in atTop, PolynomialSmoothingTwo p P J
      (smoothingExponentOne J) (smoothingExponentTwo J α) (smoothingExponentThree J) x := by
  obtain ⟨C,hC,hTV⟩ := uniform_actual_smoothing_two p P α hP hα hαp hα1
...
theorem polynomially_accurate_smoothing (p P J : Real) (hp : 0 < p) (hP : p ≤ P) (hJ : 0 < J) :
    ∃ u₁ > 0, ∃ u₂ > 0, ∃ u₃ > 0, ∀ᶠ x : Real in atTop,
      PolynomialSmoothingOne p P J u₁ u₂ u₃ x ∧ PolynomialSmoothingTwo p P J u₁ u₂ u₃ x := by
  ...
  exact ⟨_,h1,_,h2,_,h3,
    (polynomial_smoothing_one p P J α hp hP hJ hα hαp hα1).and
      (polynomial_smoothing_two p P J α hp hP hJ hα hαp hα1)⟩

#print axioms polynomially_accurate_smoothing
